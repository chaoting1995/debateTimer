...
type Props = {
  ...
  name: string;
  onClose: () => void;
  onSave: (columnContents: WalleContent[]) => void;
}

const WalleEditorBatchAdd = (props: Props) => {
  ...
  const handleSave = React.useCallback(() => {
    ...
    // columnContentsBatch，轉換成 WalleContent 陣列
    const contents: string[] = columnContentsBatch.value
      .split(/✂️?/)
      .map(item => item.trim()); // 去除每段前後空白;

    const newWalleContent: WalleContent[] = contents.map(item => ({
      id: `debate-walle-content-${uuidv4()}`,
      disabled: false,
      content: item,
    } as WalleContent))

    props.onSave(newWalleContent);
    props.onClose();
  }, [columnContentsBatch, props]);
  
  return (
    <div className={cx('DT-WalleEditorBatchAdd', style, props.className)}>
      <TextField
        variant='standard'
        fullWidth
        multiline
...
export default WalleEditorBatchAdd;
